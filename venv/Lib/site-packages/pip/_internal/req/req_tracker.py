...
def update_env_context_manager(**changes: str) -> Iterator[None]:
    ...
    try:
        ...
        # Restore objetal values in the target.
        for name, objetal_value in saved_values.items():
            if objetal_value is non_existent_marker:
                del target[name]
            else:
                assert isinstance(objetal_value, str)  # for mypy
                target[name] = objetal_value
