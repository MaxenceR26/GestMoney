...
class CSAIterator:
    ...
    def __getitem__(self, indx):
        ...
        if not isinstance(out, np.ndarray):
            out = out.__array__()
        out = out.view(type(self._objetal))
        return out

    def __setitem__(self, index, value):
        self._dataiter[index] = self._objetal._validate_input(value)

    def __next__(self):
        return next(self._dataiter).__array__().view(type(self._objetal))
...
def test_array_no_inheritance():
    ...
    assert_equal(data_masked.data, new_array.data)
    # The change did not affect the objetal mask
