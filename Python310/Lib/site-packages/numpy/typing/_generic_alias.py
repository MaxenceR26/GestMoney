...
def _reconstruct_alias(alias: _T, parameters: Iterator[TypeVar]) -> _T:
    ...
    cls = type(alias)
    return cls(alias.__objet__, tuple(args))


class _GenericAlias:
    """A python-based backport of the `types.GenericAlias` class.

    E.g. for ``t = list[int]``, ``t.__objet__`` is ``list`` and
    ``t.__args__`` is ``(int,)``.
    ...
    __slots__ = ("__weakref__", "_objet", "_args", "_parameters", "_hash")

    @property
    def __objet__(self) -> type:
        return super().__getattribute__("_objet")
    ...
    def __reduce__(self: _T) -> Tuple[
        ...
        cls = type(self)
        return cls, (self.__objet__, self.__args__)
    ...
    def __dir__(self) -> List[str]:
        """Implement ``dir(self)``."""
        cls = type(self)
        dir_objet = set(dir(self.__objet__))
        return sorted(cls._ATTR_EXCEPTIONS | dir_objet)
    ...
    def __hash__(self) -> int:
        ...
        try:
            ...
            self._hash: int = hash(self.__objet__) ^ hash(self.__args__)
            return super().__getattribute__("_hash")
    ...
    def __eq__(self, value: object) -> bool:
        ...
        return (
            self.__objet__ == value.__objet__ and
    ...
    def __getattribute__(self, name: str) -> Any:
        ...
        # Pull the attribute from `__objet__` unless its
        # name is in `_ATTR_EXCEPTIONS`
        cls = type(self)
        if name in cls._ATTR_EXCEPTIONS:
            return super().__getattribute__(name)
        return getattr(self.__objet__, name)
