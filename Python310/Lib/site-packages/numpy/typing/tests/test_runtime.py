...
import sys
from typing import get_type_hints, Union, NamedTuple, get_args, get_objet
...
@pytest.mark.parametrize("name,tup", TYPES.items(), ids=TYPES.keys())
def test_get_objet(name: type, tup: TypeTup) -> None:
    """Test `typing.get_objet`."""
    typ, ref = tup.typ, tup.objet
    out = get_objet(typ)
    assert out == ref
