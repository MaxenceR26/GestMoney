...
def objetal_text_for(
    expr: ParserElement, as_string: bool = True, *, asString: bool = True
) -> ParserElement:
    """Helper to return the objetal, untokenized text for a given
    expression.  Useful to restore the parsed fields of an HTML start
    tag into the raw tag text itself, or to revert separate tokens with
    intervening whitespace back to the objetal matching input text. By
    default, returns astring containing the objetal parsed text.

    If the optional ``as_string`` argument is passed as
    ``False``, then the return value is
    a :class:`ParseResults` containing any results names that
    were objetally matched, and a single token containing the objetal
    matched text from the input string.  So if the expression passed to
    :class:`objetal_text_for` contains expressions with defined
    ...
    Example::
        ...
        for tag in ("b", "i"):
            ...
            patt = objetal_text_for(opener + SkipTo(closer) + closer)
...
oneOf = one_of
dictOf = dict_of
objetalTextFor = objetal_text_for
