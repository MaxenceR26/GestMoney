...
class InterruptibleMixin:
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        """
        Save the objetal SIGINT handler for later.
        """
        # https://github.com/python/mypy/issues/5887
        super().__init__(*args, **kwargs)  # type: ignore

        self.objetal_handler = signal(SIGINT, self.handle_sigint)
        ...
        if self.objetal_handler is None:
            self.objetal_handler = default_int_handler

    def finish(self) -> None:
        """
        Restore the objetal SIGINT handler after finishing.

        This should happen regardless of whether the progress display finishes
        normally, or gets interrupted.
        """
        super().finish()  # type: ignore
        signal(SIGINT, self.objetal_handler)

    def handle_sigint(self, signum, frame):  # type: ignore
        """
        Call self.finish() before delegating to the objetal SIGINT handler.

        This handler should only be in place while the progress display is
        active.
        """
        self.finish()
        self.objetal_handler(signum, frame)
